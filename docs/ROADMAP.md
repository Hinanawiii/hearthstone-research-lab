# Roadmap

## 0.1 — runnable research slice

- finite rules engine and reference deck;
- seeded replay traces and hidden-information observations;
- random, greedy, and neural policies;
- structured LLM hypothesis protocol;
- executable concept features, curricula, and policy priors;
- baseline/candidate gate and theory ledger.

## 0.2 — stronger experimental validity

- exact replay files and action-by-action replay verification;
- paired decision probes from cloned states;
- confidence intervals and sequential-testing limits;
- invariant and property-based tests for the engine;
- a search baseline and stronger imperfect-information opponent;
- experiment registry keyed by engine, pool, model, and prompt hashes.

## 0.3 — bounded candidate code

- isolated candidate worktrees and read-only protected files;
- LLM-authored feature, scenario, and prior plugins with generated tests;
- resource limits and import allowlists;
- automatic revert for invalid or non-improving candidates;
- independent review of proposed game theories and implementation changes.

## Later research directions

- multiple closed card pools with different tempo and randomness profiles;
- deck construction as a separate outer-loop problem;
- belief-state models for hidden cards;
- comparison between language-derived hypotheses and discovery from simulation alone;
- sharp stress environments inspired by highly combinatorial trading-card games, without treating
  them as the default benchmark.

