...
`250339`。只有标准环境的可收藏卡会进入制卡队列；来源目录同时收录历代构筑系列的卡牌
定义。核心卡沿用旧系列 ID 的衍生牌，以及被效果直接生成的旧版可收藏牌，都可以按 ID
查询，但不会占用人工队列。
...
实现母卡时，制卡代理必须同时登记它直接引用的衍生牌。生成核验材料前，系统会检查每张
衍生牌的运行时定义和来源目录记录，缺少任何一项都会停止生成。固定数值和关键词的简单
衍生牌随母卡一起核对，不单独进入人工队列。带有动态选择、随机池或特殊状态的复杂衍生牌
需要单独做实验核验，不能凭母卡的简单场景直接准出。
...
接口会再次核对每张卡的资格，并返回实际批准数量和卡牌 ID。重复调用不会产生重复事件。
